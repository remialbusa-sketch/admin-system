...
class AccountCreatedNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail(mixed $notifiable): MailMessage
    {
        ...
        $mail = (new MailMessage)
            ->subject("Your {$roleLabel} account on ".config('app.name'))
            ->greeting("Hi {$this->user->name},")
            ->line('A Superadmin has created an account for you on '.config('app.name').'. You can sign in right away — your email is already verified.')
            ->line("**Role:** {$roleLabel}")

...
- **Resend flow:** the per-row actions cell now has a "Resend credentials" button so a Superadmin can re-send the welcome email to an existing user (e.g. they lost it). The resend is throttled to once every 5 minutes per recipient (tracked by the new `users.credentials_resent_at` column) and the email body adapts to point the user at the forgot-password flow since the original hashed password cannot be recovered. New regression tests cover success, throttle, post-throttle success, and mailer-failure paths.
- New regression coverage: `UserManagementTest` adds `test_created_account_is_marked_verified_immediately`, `test_created_account_dispatches_credentials_notification`, `test_user_still_created_when_notification_dispatch_fails`, `test_superadmin_can_resend_credentials_to_existing_user`, `test_resend_is_throttled_to_once_every_five_minutes`, `test_resend_succeeds_after_throttle_window_passes`, and `test_resend_keeps_user_intact_when_mailer_throws`.
